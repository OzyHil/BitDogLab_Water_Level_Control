...
struct tcp_pcb *init_tcp_server() {
    ...
    // Tenta vincular à porta 80, retorna nulo se falhar
    if (tcp_bind(server, IP_ADDR_ANY, 80) != ERR_OK) return NULL;
    ...
    if (strstr(request, "GET /data")) {
        ...
        *ptr = '\0';
        ...
        if (body) {
            ...
            if (min_str) min = atoi(min_str + 6); // pula '"min":'
            if (max_str) max = atoi(max_str + 6); // pula '"max":'
